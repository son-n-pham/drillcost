...
const BitsPanel: React.FC<BitsPanelProps> = ({ bits, setBits, depthUnit }) => {
  ...
  // Touch support
  const touchBitSelection = useTouchSelection<string | null>(null);
  const isTouch = isTouchDevice();
  const touchStartTime = useRef<number>(0);
  const touchMoved = useRef<boolean>(false);

  const handleBitTouchStart = useCallback((e: React.TouchEvent, bitId: string) => {
    touchStartTime.current = Date.now();
    touchMoved.current = false;
  }, []);

  const handleBitTouchMove = useCallback((e: React.TouchEvent) => {
    touchMoved.current = true;
  }, []);

  const handleBitTouchEnd = useCallback((e: React.TouchEvent, bitId: string) => {
    const duration = Date.now() - touchStartTime.current;
    if (!touchMoved.current && duration < 300) {
      // Quick tap - toggle selection
      touchBitSelection.select(bitId);
    }
  }, [touchBitSelection]);
  ...
  return (
    <div className="card transition-colors duration-300">
      ...
      <div className="p-3 grid grid-cols-2 md:grid-cols-1 xl:grid-cols-2 gap-2">
        {bits.map((bit) => {
          const isTouchSelected = touchBitSelection.isSelected(bit.id);
          return (
            <div
              key={bit.id}
              className={clsx(
                "p-2 border border-slate-200 dark:border-[var(--bh-border)] rounded-lg bg-white dark:bg-[var(--bh-surface-0)] relative group hover:border-blue-300 dark:hover:border-[var(--bh-primary)] transition-colors shadow-sm",
                isTouchSelected && "ring-2 ring-blue-400"
              )}
              onTouchStart={(e) => handleBitTouchStart(e, bit.id)}
              onTouchMove={handleBitTouchMove}
              onTouchEnd={(e) => handleBitTouchEnd(e, bit.id)}
            >
              <div className={clsx(
                "absolute -top-1.5 -right-1.5 transition-all z-10",
